...
//         @Query("SELECT e FROM EmploiDuTemps e WHERE e.cin = :cin AND e.date >= CURRENT_DATE ORDER BY e.date ASC, e.heure_debut ASC")
//         List<EmploiDuTemps> findByCinOrderByDateAscHeure_debutAsc(@Param("cin") String cin);
        
//         @Query("SELECT e FROM EmploiDuTemps e WHERE e.date >= CURRENT_DATE ORDER BY e.date, e.heure_debut")
