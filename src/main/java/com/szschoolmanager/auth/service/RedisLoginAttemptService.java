...
/**
 * Service de gestion des tentatives de connexion basé sur Redis. Utilise un script Lua atomique
 * (INCR + EXPIRE) fourni par RedisConfig.rateLimitScript.
 */
@Slf4j
@Service
...
public class RedisLoginAttemptService {
    ...
    try {
      Long current =
          redisTemplate.execute(
              ...
              String.valueOf(
                  MAX_FAILED_ATTEMPTS), // limit argument non utilisé dans our script but kept for
              // compatibility
      ...
      log.error(
          "Redis unavailable while recording login attempt for {} — allowing attempts (fail-open)",
          username,
          ex);
      return 0L;
      ...
      log.error(
          "Redis data access error while recording login attempt for {} — allowing attempts",
          username,
          ex);
      return 0L;
    } catch (Exception ex) {
      log.error(
          "Unexpected error while recording login attempt for {} — allowing attempts",
          username,
          ex);
      return 0L;
    }
  ...
  public boolean isLocked(String username) {
    ...
    try {
      ...
      meterRegistry.counter("auth.login.redis.unavailable").increment();
      log.error(
          "Redis unavailable while checking lock for {} — treat as not locked (fail-open)",
          username,
          ex);
      return false;
    } catch (Exception ex) {
  ...
  public void resetAttempts(String username) {
    ...
    try {
      ...
      meterRegistry.counter("auth.login.redis.unavailable").increment();
      log.error(
          "Redis unavailable while resetting attempts for {} — nothing done (fail-open)",
          username,
          ex);
    } catch (Exception ex) {
